...
class YSBangumiDetailHeadCharacterView: UIView {
    ...
    var headDetailModel: YSBangumiDetailModel? {
        didSet {
            ...
            if let weakDay = headDetailModel?.weekday {
                updateTimeLabel.text = "连载中,每周\(weakDay.weekDay())更新"
            }
        }
    }
    ...
    class func instanceView() -> YSBangumiDetailHeadCharacterView {
        let bundName = String(describing: self)
        return Bundle.main.loadNibNamed(bundName, owner: nil, options: nil)?.last as! YSBangumiDetailHeadCharacterView
    }
}
