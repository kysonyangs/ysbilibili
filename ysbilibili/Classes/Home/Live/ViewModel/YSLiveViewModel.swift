...
@objc protocol YSLiveViewModelDelegate {
    @objc optional func liveViewModelReloadSetion(section:Int)
}
...
extension YSLiveViewModel {
    ...
    func requestDatas(finishCallBack:@escaping ()->(),failueCallBack:@escaping ()->()){
        ...
        YSNetworkTool.shared.requestData(.get, URLString: "http://live.bilibili.com/AppNewIndex/common?scale=2&device=phone&platform=ios", finished: { (result) in
            // <1. 获取到dict
            ...
            if let bannerArray = JSONDeserializer<YSLiveBannerModel>.deserializeModelArrayFrom(json: bannerJsonString){
                self.bannerModelArray = bannerArray as? [YSLiveBannerModel]
            }
                // <<2.普通的数据
            if let commonArray = JSONDeserializer<YSLiveItemModel>.deserializeModelArrayFrom(json: areaJsonString){
                commonAreaArray = commonArray as? [YSLiveItemModel]
            }
            ...
            if let commonAreaArray = commonAreaArray{
                for itemModel in commonAreaArray {
                    ...
                    // <<2. 拿到前四组数据
                    var tempLives = [YSLiveDetailModel]()
                    for i in 0..<4{
                        let model = lives[i]
        ...
        // 4. 请求推荐主播的数据
        group.enter()
        YSNetworkTool.shared.requestData(.get, URLString: "http://live.bilibili.com/AppNewIndex/recommend?actionKey=appkey&appkey=27eb53fc9058f8c3&build=3970&buvid=937484d7e0997f66a821b1e335018212&device=phone&mobi_app=iphone&platform=ios&scale=2&sign=26d2146bb9aa27fcb847e5f50ff9700f&ts=1479285261", finished: { (result) in
            // <1. 获取到dict
            let jsonDict = JSON(result)
            // <2. 转成string
            let modelJson = jsonDict["data"]["recommend_data"].dictionaryObject
            // <3. 转模型
            if let hotModel = JSONDeserializer<YSLiveItemModel>.deserializeFrom(dict: modelJson as NSDictionary?){
    ...
    fileprivate func reloadSectionDataWithType(type: String?,section: Int) {
        ...
        YSNetworkTool.shared.requestData(.get, URLString: urlString, finished: { (result) in
            ...
            if type == "hot" {// 1. 推荐的数据
                ...
                // <3. 转模型
                let hotModel = JSONDeserializer<YSLiveItemModel>.deserializeFrom(dict: modelJson as NSDictionary?)
                // <4. 处理成展示的array排列
                ...
                // <3. 转成model
                let commonModelArray = JSONDeserializer<YSLiveDetailModel>.deserializeModelArrayFrom(json: resultString)
                // <4. 处理长度 每个row4行
                guard let count = commonModelArray?.count else {return}
                guard  count > 4 else{return}
                var tempArray = [YSLiveDetailModel]()
                for i in 0..<4{
            ...
            if let delegate = self.delegate {
                if let method = delegate.liveViewModelReloadSetion{
                    method(section)
                }
            }
        }) { (error) in
