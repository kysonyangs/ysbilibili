...
class ZHNhomePageViewController: YSBaseViewController {
...
extension ZHNhomePageViewController: UITableViewDelegate,UITableViewDataSource {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let max = kbackImageHeight - kNavBarHeight
...
extension ZHNhomePageViewController {
    fileprivate func setupUI() {
        ...
        let backImage = UIImage(named: "common_back_v2")?.ysTintColor(UIColor.black)
...
extension ZHNhomePageViewController {
    ...
    @objc func choseToPlay(notifocation: Notification) {
        ...
        playerVC.itemModel = item as? YSItemDetailModel
        self.navigationController?.pushViewController(playerVC, animated: true)
    }
    
    @objc func selectedBangumi(notification: Notification) {
        /// 数据有问题不知为何
        let itemModel = notification.userInfo?[kBangumiModelKey] as! YSHomeBangumiDetailModel
