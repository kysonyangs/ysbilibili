...
extension ZHNPlayerBaseViewController {
    ...
    func resignFullScreen() {
        ...
        animationImageView.transform = animationImageView.transform.rotated(by: CGFloat(Double.pi / 2))
        // 3. 还原player的位置
    ...
    @objc func fullScreenAction() {
        ...
        UIView.animate(withDuration: 0.3, animations: {
            ...
            DispatchQueue.afer(time: 0.1, action: {
                ...
                // <2. 再旋转过来
                self.liveContainerView.transform = self.liveContainerView.transform.rotated(by: CGFloat(Double.pi / 2))
                // <3. 移除
                animationImageView.removeFromSuperview()
