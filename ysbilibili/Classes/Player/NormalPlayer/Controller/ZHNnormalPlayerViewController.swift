...
class ZHNnormalPlayerViewController: ZHNPlayerBaseViewController {
    ...
    // MARK - 属性
    /// 数据的模型
    var itemModel: YSItemDetailModel?
    /// 判断是否需要滑动的效果
    var watced = false
    /// viewmodel
    ...
    lazy var blurView: playerBlurView = {
        ...
        blurView.tapAction = {[weak self] in
            self?.pLoadPlay()
            YSNotificationHelper.shutDownOldPlayer()
        }
        return blurView
    }()
    lazy var slideMenuView: SlideMenu = { [unowned self] in
        ...
        // 4.生成slidemenu
        let menu = SlideMenu(frame: rect, titles: [" 简介        "," 评论        "], padding: 15, normalColor: normalColor, highlightColor: hightLightColor, font: 16, sliderColor: kNavBarColor, scrollView: self.contentScrollView, isHorizon: true, rowHeight: 30)
        return menu
    }()
...
extension ZHNnormalPlayerViewController {
    fileprivate func setupUI() {
        ...
        detailController.view.frame = CGRect(x: 0, y: -20, width: kScreenWidth, height: contentScrollView.ysHeight)
        commondController.view.frame = CGRect(x: kScreenWidth, y: -20, width: kScreenWidth, height: contentScrollView.ysHeight)
    ...
    fileprivate func pLoadPlay() {
        ...
        self.watced = true
        // <<2. 简介评论的处理
        self.slideMenuContainer.ysY = knormalPlayerHeight + kinputDanmuHeight
        self.contentScrollView.ysY = knormalPlayerHeight + kinputDanmuHeight + kslideMenuViewHeight
        self.detailController.view.ysHeight = kScreenHeight - kinputDanmuHeight - kslideMenuViewHeight - knormalPlayerHeight
        self.commondController.view.ysHeight = kScreenHeight - kinputDanmuHeight - kslideMenuViewHeight - knormalPlayerHeight
...
extension ZHNnormalPlayerViewController {
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if keyPath == "contentOffset" {
            ...
            blurView.ysHeight = delta
                //<<2. slidemenu 和地下的view会随着改变
            if isinputShowing {
                delta = delta + kinputDanmuHeight
            }
            slideMenuContainer.ysY =  delta
            contentScrollView.ysY = delta+kslideMenuViewHeight
        }
    }
